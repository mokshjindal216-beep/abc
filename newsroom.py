...
def generate_content(article):
    ...
    # Strictly enforcing word counts in prompts
    hl = client.chat.completions.create(messages=[{"role":"user","content":f"Write a 5-8 word viral headline for: '{article['title']}'. UPPERCASE. No quotes. Aggressive."}], model=model).choices[0].message.content.strip().replace('"','')
    summ = client.chat.completions.create(messages=[{"role":"user","content":f"Summarize in EXACTLY 20-25 words. Article: '{article['title']}'. Text only."}], model=model).choices[0].message.content.strip()
...
def create_video(article, headline, summary):
    ...
    # Safe Zone: Bottom 30% space (1920 * 0.7 = 1344)
    SAFE_BOTTOM_LIMIT = 1344
    ...
    for line in textwrap.wrap(summary, width=30):
        if cursor > SAFE_BOTTOM_LIMIT: break # Stops text from entering UI danger zone
